...
const Checkout = () => {
  ...
  return (
    <div className='h-screen bg-[#EAEDED]'>
          ...
          <div className='col-span-6 bg-white'>
            <div className='text-2xl xl:text-3xl m-4'>Shopping Cart</div>
            {
              products.map(product => {
                return (
                  <div key={product.id}>
                    <div className='grid drid-cols-12 divide-y divide-gray-400 mr-4'>
                      <div className='col-span-10 grid grid-cols-8 divide-y divide-gray-400'> 
                        ...
                        <div>
                          <button className='text-sm xl:text-base font-semibold rounded
                          text-blue-500 mt-2 mb-1' onClick={() => dispatch(removeFromCart(product.id))}>Delete</button>
                        </div>
                        <div className='grid grid-cols-3 w-20 text-center'>
                          ...
                          <div className='text-sl xl:text-2xl bg-gray-400 rounded'>+</div>
                        </div>
                        </div>
            ...
            <div className='text-lg xl:text-xl text-right mb-4 mr-4'>Subtotal ({itemNumber} items): 
              <span className='font-semibold'>{GB_CURRENCY.format(subtotal)}</span></div>
          </div>
          {/* Checkout */}
          <div className='col-span-2 bg-white rounded h-[250px] p-2'>
              <div className='text-xs xl:text-sm text-green-300 mb-2'>Your order qualifies for 
                <span className='font-bold'>FREE DELIVERY</span>. Delivery Details</div>
              <div className='text-base xl:text-lg mb-4'>Subtotal ({itemNumber} items): 
              <span className='font-semibold'>{GB_CURRENCY.format(subtotal)}</span></div>
