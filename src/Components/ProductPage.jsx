    ...
    <div className='h-screen bg-[#EAEDED]'>
        <div className='min-w-[1000px] max-w[[1500px] m-auto p-4'>  
          <div className='grid grid-cols-10 gap-2'>
            ...
            {/*Right */}
            <div className='col-span-2 p-2 rounded bg-white'>
              <div className='text-xl xl:text-2xl text-red-700 text-right font-semibold'>{GB_CURRENCY.format(product.price)}</div>
              <div className='text-base xl:text-lg text-grey-500 text-right font-semibold'>RRP:<span className='line-through'>{product.oldPrice}</span></div>
              ...
              <div className='text-base xl:text-lg'>Quantity:
                  <select onChange={(e) => setQuantity(e.target.value)} className='p-2 bg-white border rounded-md focus:border-indigo-600'>
                    ...
                    hover:bg-yellow-140 mt-3'>Add to Cart</button>
                    </Link>
            </div>
          </div>
        </div>
